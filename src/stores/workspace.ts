  ...
  actions: {
    ...
    fetchExperiments() {
      return ApiService.getExperimentsApiV1ExperimentsGet().then(response => {
        ...
        if (response.length > 0 && this.experiment == null || !availableExperimentNames.includes(this.currentExperiment.name)) {
    ...
    setExperiment(experimentName: string) {
        ...
        return ApiService.getLatestExperimentApiV1ExperimentsExperimentNameLatestGet(this.experiment.name).then(response => {
          this.setCurrentResult(response);
        });
    },
    setExperimentVersion(experimentName: string, version: number) {
      this.experiment = this.experiments.find(experiment => experiment.name === experimentName);
      return ApiService.getExperimentVersionApiV1ExperimentsExperimentNameVersionVersionGet(this.experiment.name, version).then(response => {
          this.setCurrentResult(response);
      });
    ...
    fetchAlgorithm(algorithmReferenceType: AlgorithmReferenceType, algorithmReference: string) {
      return ApiService.getAlgorithmApiV1AlgorithmReferenceTypeReferenceGet(algorithmReferenceType, algorithmReference).then(response => {
