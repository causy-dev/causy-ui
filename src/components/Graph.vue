  ...
  methods: {
    ...
    edgeConfig(edge: EdgeInterface, edgeConfigs: Record<string, any>) {
      ...
      for (let conditionalConfig of edgeConfigs[edge.edge_type.name].conditional_ui_configs) {
        if (
          conditionMapping[conditionalConfig.condition_comparison](
            edge.metadata[conditionalConfig.condition_field],
            conditionalConfig.condition_value
          )
        ) {
  ...
  computed: {
    elements(): Element[] {
      ...
      if (
        ...
        this.graph.nodes === undefined ||
        this.graph.edges === undefined ||
        this.graph.algorithm === undefined ||
        ...
        let handlePositions = this.findBestHandle(
          {
            ...
            nodeId: '',
            type: undefined,
            x: layout[edge.u.id].x,
            y: layout[edge.u.id].y,
            width: 200,
            height: 100
          },
          {
            ...
            nodeId: '',
            type: undefined,
            x: layout[edge.v.id].x,
            y: layout[edge.v.id].y,
            width: 200,
            height: 100
          }
        )
